...
Responsabilités :
  ...
  - Démarrer Flask dans un thread daemon
  - Attendre que le serveur soit prêt
  - Ouvrir la fenêtre WebView

Logging : fichier launcher.log dans le même dossier que l'exe (rotation 1 Mo).
"""
...
# ── AppID Windows (barre des tâches) ─────────────────────────────────────────
...
def _setup_logging() -> logging.Logger:
    """Logger avec rotation → launcher.log (1 Mo max, 2 fichiers de backup)."""
...
def ensure_persistent_webview_storage() -> None:
    """Force WebView2 à utiliser un profil persistant dédié (Windows uniquement)."""
...
def wait_for_server(host: str, port: int, timeout_sec: float = 15.0) -> bool:
    """Poll le serveur jusqu'à disponibilité ou timeout."""
...
def _run_flask(port: int) -> None:
    """Lance Flask dans le thread courant (bloquant). Import tardif : freeze-safe."""
    try:
...
def _show_error_window(message: str) -> None:
    """Affiche une fenêtre WebView avec le message d'erreur."""
...
def main() -> None:
    ...
    try:
        ...
        if not ready:
            ...
            _show_error_window(
                "Le serveur local n'a pas démarré dans le délai imparti.\n\n"
                f"Port essayé : {port}\n"
                f"URL : {url}\n\n"
                "👉 Consulte launcher.log (à côté de l'exe) "
                "et copie la section FLASK_ERROR."
            )
            return

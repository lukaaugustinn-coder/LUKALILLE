...
try:
    ...
    def _push_state_async(state: dict[str, Any]) -> None:
        """No-op fallback."""
...
def load_state() -> dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(raw, dict):
            log.warning("state.json corrompu — réinitialisation.")
            s = default_state()
            save_state(s)
            return s
...
def compute_time_bins_stats(sales: list[dict[str, Any]]) -> list[dict[str, Any]]:
    BINS: list[tuple[str, int, int]] = [
        ("08:00-10:00", 8, 10), ("10:00-12:00", 10, 12), ("12:00-14:00", 12, 14),
        ("14:00-16:00", 14, 16), ("16:00-18:00", 16, 18), ("18:00-20:00", 18, 20),
    ]
...
def compute_multi_vad(sales: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "count_multi": count_multi, "count_solo": count_solo,
        "total": total, "pct_multi": round(count_multi / total * 100, 1) if total > 0 else 0.0,
    }
...
def draw_pie_chart(
    c: canvas.Canvas, cx: float, cy: float, r: float,
    items: list[tuple[str, int]], total: int, palette: list[Any],
    ...
    for col, _label, count, pct in legend:
        ...
        if sweep >= 24:
            ...
            c.drawCentredString(
                ...
                cy + r * 0.62 * math.sin(rad) - 3, f"{pct:.0f}%",
            )
    ...
    def assign_column_y(items_list):
        n = len(items_list)
        if not n: return []
        y_start = cy + (n - 1) * LINE_H / 2.0
        sorted_items = sorted(items_list, key=lambda it: -math.sin(math.radians(it[4])))
        return [(*item, y_start - row * LINE_H) for row, item in enumerate(sorted_items)]

    def draw_label_item(_i, col, label, pct, mid_a, label_y):
        ...
        c.setFillColor(INK_PDF)
        if cos_a >= 0: c.drawString(col_x + 5, label_y, disp)
        else:          c.drawRightString(col_x - 5, label_y, disp)

    for item in assign_column_y(right_items): draw_label_item(*item)
    for item in assign_column_y(left_items):  draw_label_item(*item)
    return legend


# ── PDF export ──────────────────────────────────────────────────
def export_pdf(
    general: int, sales: list[dict[str, Any]], rebonds: list[dict[str, Any]],
    title: str = "RECAP VENTES", sg_agenda: int = 0,
) -> str:
    ...
    def draw_header_block(heading: str, subtitle: str = "") -> None:
        HDR_H = 60
        cv.setFillColor(INK); cv.rect(0, H - HDR_H, W, HDR_H, fill=1, stroke=0)
        cv.setFillColor(colors.HexColor("#C9A000")); cv.rect(0, H - HDR_H, 4, HDR_H, fill=1, stroke=0)
        cv.setFillColor(WHITE); cv.setFont("Helvetica-Bold", 20); cv.drawString(16, H - 28, "T850")
        cv.setFillColor(SILVER_D); cv.setFont("Helvetica", 7.5); cv.drawString(16, H - 40, "COMPTEUR VAD")
        cv.setFillColor(colors.HexColor("#3A5070")); cv.rect(118, H - HDR_H + 8, 1, HDR_H - 16, fill=1, stroke=0)
        cv.setFillColor(WHITE); cv.setFont("Helvetica-Bold", 12); cv.drawString(130, H - 26, heading[:55])
        if subtitle:
            cv.setFillColor(SILVER_D); cv.setFont("Helvetica", 7.5); cv.drawString(130, H - 38, subtitle[:80])
        ...
        cv.setFillColor(colors.HexColor("#2A4060")); cv.rect(0, H - HDR_H - 1, W, 1, fill=1, stroke=0)
    ...
    def draw_footer(page_num: int = 0) -> None:
        ...
        if page_num: cv.drawRightString(W - 20, 12, f"Page {page_num}")
    ...
    def table_row(cols_data, y_pos, row_index, col_defs) -> float:
        row_h = 14
        if row_index % 2 == 0:
    ...
    SALES_COLS: list[tuple[str, int, int, str]] = [
        ("#", 20, 18, "center"), ("DATE / HEURE", 42, 100, "left"),
        ("PRODUIT", 146, 110, "left"), ("CATÉGORIE", 260, 90, "left"),
        ("CLIENT", 354, 120, "left"), ("NÉ(E) LE", 478, 75, "left"),
    ]

    page_num = 1
    n_sales, n_rebonds = len(sales or []), len(rebonds or [])
    draw_header_block(title, f"Export du {now.strftime('%d/%m/%Y à %H:%M')}  ·  {n_sales} vente(s)  ·  {n_rebonds} rebond(s)")
    ...
    for i, (lbl, val) in enumerate(kpi_boxes):
        ...
        cv.setStrokeColor(SILVER); cv.setLineWidth(0.6); cv.rect(bx, kpi_y - 22, box_w - 4, 28, fill=0, stroke=1)
    ...
    for s in sales or []:
        if y < 50:
            ...
            draw_header_block(f"{title}  (suite p.{page_num})"); draw_footer(page_num)
            y = CONTENT_TOP; y = section_title("Détail des ventes (suite)", y)
    ...
    if rebonds:
        if y < 120:
            draw_footer(page_num); cv.showPage(); page_num += 1
            draw_header_block(f"{title}  (suite p.{page_num})", "Rebonds"); draw_footer(page_num)
            y = CONTENT_TOP
        y -= 10
        REBOND_COLS: list[tuple[str, int, int, str]] = [
            ("#", 20, 18, "center"), ("DATE / HEURE", 42, 110, "left"),
            ("PRODUIT", 156, 120, "left"), ("CATÉGORIE", 280, 100, "left"),
        ]
        ...
        for ri, rb in enumerate(rebonds):
            if y < 50:
                draw_footer(page_num); cv.showPage(); page_num += 1
                draw_header_block(f"{title}  (suite p.{page_num})", "Rebonds (suite)"); draw_footer(page_num)
                y = CONTENT_TOP; y = section_title("Détail des rebonds (suite)", y)
                y = table_header(REBOND_COLS, y)
            row_data = [
                (str(ri + 1), SILVER_D),
    ...
    if sg_agenda:
        if y < 80:
            draw_footer(page_num); cv.showPage(); page_num += 1
            draw_header_block(f"{title}  (suite p.{page_num})"); draw_footer(page_num); y = CONTENT_TOP
        y -= 8
        cv.setFillColor(colors.HexColor("#1A3A5F")); cv.rect(20, y - 4, W - 40, 20, fill=1, stroke=0)
        cv.setFillColor(colors.HexColor("#C9A000")); cv.rect(20, y - 4, 4, 20, fill=1, stroke=0)
    ...
    legy -= 4; cv.line(LEG_X, legy, LEG_X + LEG_W, legy); legy -= 10

    for row_i, (col, label, count, pct) in enumerate(legend_entries):
        if legy < PIE_CY - PIE_R - 10: break
        if row_i % 2 == 0:
            cv.setFillColor(ROW_ALT); cv.rect(LEG_X, legy - 3, LEG_W, ROW_H_L - 1, fill=1, stroke=0)
        cv.setFillColor(col)
    ...
    legy -= 3
    cv.setStrokeColor(SILVER); cv.setLineWidth(0.5); cv.line(LEG_X, legy + 6, LEG_X + LEG_W, legy + 6)
    ...
    for bi, b in enumerate(bins):
        ...
        y = table_row([(lbl, INK), (str(cnt), ACCENT), (f"{pct:.1f}%", INK), (perf, pcol)], y, bi, TIME_COLS)
        if y < 80: break

    y -= 12
    if y < 150:
        draw_footer(page_num); cv.showPage(); page_num += 1
        draw_header_block("ANALYSE DES VENTES  (suite)"); draw_footer(page_num); y = CONTENT_TOP
    ...
    for ri, (p, rb, sl, conv) in enumerate(compute_product_conversion_table(sales or [], rebonds or [])):
        if y < 60:
            draw_footer(page_num); cv.showPage(); page_num += 1
            draw_header_block("ANALYSE DES VENTES  (suite)"); draw_footer(page_num); y = CONTENT_TOP
            y = section_title("Taux de conversion par produit (suite)", y); y = table_header(CONV_COLS, y)
        ...
        y = table_row([(safe_str(p)[:36], INK), (str(rb), SILVER_D), (str(sl), ACCENT), (conv_txt, conv_col)], y, ri, CONV_COLS)
...
def api_save() -> Any:
    ...
    state = load_state()
    payload: dict[str, Any] = data.get("state", data) if "state" in data and isinstance(data.get("state"), dict) else data
...
def export_pdf_route() -> Any:
    try:
        ...
        return jsonify({"ok": True, "path": path, "folder": os.path.dirname(path), "filename": os.path.basename(path)})
...
def export_csv_route() -> Any:
    """Génère et renvoie un fichier CSV UTF-8 BOM des ventes et rebonds."""
    try:
        data    = request.get_json(silent=True) or {}
        sales   = data.get("sales") or []
        rebonds = data.get("rebonds") or []
        ...
        buf = io.StringIO()
        buf.write("﻿")  # UTF-8 BOM pour Excel
        w = csv.writer(buf, delimiter=";", quoting=csv.QUOTE_MINIMAL)

        # Ventes
        w.writerow(["TYPE", "DATE", "CATÉGORIE", "PRODUIT", "NOM", "PRÉNOM", "NAISSANCE"])
        for s in sales:
            cli = s.get("client") or {}
            w.writerow([
                "VENTE",
                safe_str(s.get("dt_display") or s.get("dt") or ""),
                safe_str(s.get("category") or ""),
                safe_str(s.get("product") or ""),
                safe_str(cli.get("nom") or ""),
                safe_str(cli.get("prenom") or ""),
                safe_str(cli.get("naissance") or ""),
            ])

        # Rebonds
        w.writerow([])
        w.writerow(["TYPE", "DATE", "CATÉGORIE", "PRODUIT", "", "", ""])
        for r in rebonds:
            w.writerow([
                "REBOND",
                safe_str(r.get("dt_display") or r.get("dt") or ""),
                safe_str(r.get("category") or ""),
                safe_str(r.get("product") or ""),
                "", "", "",
            ])

        csv_bytes = buf.getvalue().encode("utf-8-sig")
        now = datetime.now()
        filename = f"t850_export_{now.strftime('%Y-%m-%d_%H-%M')}.csv"
        return Response(
            csv_bytes,
        ...
        log.exception("Échec de l'export CSV.")
        return jsonify({"ok": False, "error": str(e)}), 500
...
def open_folder_route() -> Any:
    try:
        ...
        if sys.platform.startswith("win"):
            ...
            import subprocess; subprocess.Popen(["open", candidate])
        else:
            import subprocess; subprocess.Popen(["xdg-open", candidate])
        return jsonify({"ok": True})
    except Exception as e:
